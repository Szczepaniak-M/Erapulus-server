...
import static pl.put.erasmusbackend.web.common.CommonRequestVariable.PAGE_QUERY_PARAM;
import static pl.put.erasmusbackend.web.common.CommonRequestVariable.PAGE_SIZE_QUERY_PARAM;
...
public class ControllerUtils {
    ...
    public static Mono<ServerResponse> withPageParams(ServerRequest request, Function<PageRequest, Mono<ServerResponse>> function) {
        return Mono.just(request.queryParam(PAGE_QUERY_PARAM))
                   .map(optional -> optional.map(Integer::parseInt)
                                            .orElse(DEFAULT_PAGE))
                   .map(page -> {
                       int size = request.queryParam(PAGE_SIZE_QUERY_PARAM)
                                         .map(Integer::parseInt)
